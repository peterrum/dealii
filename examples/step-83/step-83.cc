...
namespace Step83
{
  ...
  // writeable), create an output archive, and then move the serialized
  ...
  void CathodeRaySimulator<dim>::checkpoint()
  {
...

    {
      ...
      AssertThrow(checkpoint_file,
                  ExcMessage("Could not write to the <checkpoint.tmp> file."));
    ...
    // At this point, the serialized data of this file has ended up in
    // the `checkpoint.tmp` file. As mentioned in the introduction, we do
    // not want to directly overwrite the final `checkpoint` file for
    // fear that the program may be interrupted *while writing the checkpoint*.
    // This would result in a corrupted checkpoint file, and defeat the whole
    // purpose of checkpointing because one cannot restart from such a file.
    // On the other hand, if we got here, we know that the `checkpoint.tmp`
    // file was successfully written, and we can rename it to `checkpoint`,
    // in the process replacing the old file.
    //
    ...
    // interrupted somewhere within this operation:
    std::filesystem::rename("checkpoint.tmp", "checkpoint");
  }
