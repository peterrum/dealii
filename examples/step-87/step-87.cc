 ...
 *
 * Authors: Magdalena Schreter-Fleischhacker, Peter Munch
 */
...
// The files most relevant for this tutorial are the one that
// contains Utilities::MPI::RemotePointEvaluation and the distributed evaluation
// functions in the VectorTools namespace that use
// Utilities::MPI::RemotePointEvaluation.
#include <deal.II/base/mpi_remote_point_evaluation.h>
#include <deal.II/numerics/vector_tools.h>

// This header file provides the class FEPointEvaluation that allows us to read
// values of a solution vector at arbitrary unit points of a cell.
#include <deal.II/matrix_free/fe_point_evaluation.h>

// In the following, we define utility functions that are used
// below in the mini examples.

// We pack everything that's specific to this program into a namespace
// of its own.
namespace Step87
{
  ...
  {
  public:
    FormattedOutput(const std::string file_name)
      : file(file_name)
    {}
    ...
    {
      file << std::fixed << std::right << std::setw(7) << std::setprecision(3)
           << output;
  ...
  // A given list of @p points and corresponding values @p values_0
  // and @p values_1 (optional), are printed columnwise to a file @p
  // file_name. In addition, the first column represents the distance
  // of the points to the first point.
  ...
  {
    FormattedOutput file(file_name);
    ...
    {
      file << points[0].distance(points[i]);
      file << " " << points[i];
  ...
  // @p support_points from a provided Mapping @p mapping and a DoFHandler
  // @p dof_handler.
  ...
  // Create a unique list of the real coordinates of support points into
  // @p support_points from a provided Mapping @p mapping and a DoFHandler
  // @p dof_handler within a narrow band. Thereto, at the support point the
  // value of the finite element function @p signed_distance with a corresponding
  // DoFHandler @p dof_handler_support_points is evaluated. The support point is
  // only collected if the absolute value is below the value for the @p narrow_band_threshold.
  ...
  // with a sequential order of the coordinates into a list of points.
  template <int spacedim>
  ...
  //
  // In this introductory example, we demonstrate basic functionalities to
  // evaluate solution quantities at arbitrary points on a serial (or locally
  // owned) mesh. For this purpose we first create the typical objects needed
  // for a finite element discretization (defined by the mapping, triangulation,
  ...
  void example_0()
  {
    ...
    // First, we create a list of arbitrary points inside the domain at which we
    // would like to evaluate the vector.
    const auto points_line =
      create_points_along_line(Point<dim>(0.0, 0.7), Point<dim>(1.0, 0.7), 20);

    // Now we loop over all evaluation points. In the first step, we determine
    // via GridTools::find_active_cell_around_point() the cell $K$ which
    ...
    for (const auto &p_real : points_line)
      {
        ...
        // Now that we determined $K$ and $\hat{\boldsymbol{x}}_K$, we can
        // perform the evaluation of the finite element solution at this
        // point. In the following, we show three possibilities for this
        // purpose. In the first approach, we follow a traditional approach
        ...
        // takes a list of unit points for subsequent evaluation.
        {
    ...
    // We output the request points together with the corresponding
    // evaluated solution to a CSV file.
    std::cout << " - writing csv file" << std::endl;
    print_along_line("ex0.csv", points_line, values_line);
  }
  ...
  //
  // @sect3{Mini example 1: Evaluation at given points for a distributed mesh}
  //
  ...
  void example_1()
  {
    ...
    // the given points (profile). The ghost values of the to be read vector
    ...
    // In addition to VectorTools::point_values(), via
    // VectorTools::point_gradient() function gradients can be evaluated.
    // However, for the computation of use-derived quantities one might need
    // to fall back to the direct usage of
    // Utilities::MPI::RemotePointEvaluation::evaluate_and_process() or
    ...
    // of the solution vector at the request points. First, we define a
    // lambda function for the operation on the surrounding cells. From the
    // CellData object we can setup a FEPointEvaluation to read the solution
    // values at the cell-specific unit coordinates of the request points.
    // Then, we assign the values to the result vector.
    ...
    // results along the line as CSV. You can import the CSV file in
    // ParaView and compare the result with the native line plot of
    // ParaView based on the VTU file. Again, the ghost values of
    // the to be read vector need to be updated from the user.
    DataOut<dim> data_out;
    data_out.add_data_vector(dof_handler, signed_distance, "solution_ls");
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
      {
        std::cout << " - writing csv file" << std::endl;
        print_along_line("ex1.csv", profile, profile_signed_distance, output);
      }
  }
  ...
  // can extrapolate the values on the interface. Note that the demonstrated
  ...
  void example_2()
  {
    ...
    // We compute an exemplary finite element solution vector
    ...
    // with a maximum distance of 0.1 from the zero-levelset isosurface.
    ...
    // absolute acceptable change in the distance in one iteration is set.
    ...
    // total number of points that need to be processed and did not
    ...
    // illustration, we export the coordinates of the to-be-updated points for
    // each iteration to a CSV file. Next, we can evaluate the signed distance
    // function and the gradient at those points to update the solution
    ...
    // and register those points that they still need to be processed.
    Utilities::MPI::RemotePointEvaluation<dim, dim> rpe;
    ...
    for (int it = 0; it < max_iter && n_unmatched_points > 0; ++it)
      {
        ...
        std::ofstream file("ex_2_" + std::to_string(it) + ".csv");
        ...
        const auto eval_values =
          dealii::VectorTools::point_values<1>(rpe,
                                               dof_handler,
                                               signed_distance);

        const auto eval_gradient =
          dealii::VectorTools::point_gradients<1>(rpe,
                                                  dof_handler,
                                                  signed_distance);
    ...
    // iterations and there are still projection points with a distance
    ...
    // can be used to update the signed distance function, i.e.
    ...
    // This might be helpful to improve accuracy for e.g.
    ...
    solution.update_ghost_values();
    const auto vals =
      dealii::VectorTools::point_values<1>(rpe, dof_handler, solution);
    solution.zero_out_ghost_values();
  ...
  // front-tracking of an immersed surface mesh $\mathbb{T}_\Gamma$ immersed
  // into an Eulerian background fluid mesh $\mathbb{T}_\Omega$. Such an
  // immersed mesh might come from a front-tracking approach @cite
  // peskin1977numerical, @cite unverdi1992front.
  //
  // The immersed surface is transported according to a prescribed velocity
  // field from the background mesh. Subsequently, we perform a sharp
  // computation of the surface-tension term:
  // @f[
  // (\boldsymbol v_i (\boldsymbol{x}), \boldsymbol F_S
  // (\boldsymbol{x}))_{\Omega}
  // =
  // \left( \boldsymbol{v}_i (\boldsymbol{x}), \sigma \kappa (\boldsymbol{x})
  // \boldsymbol{n} (\boldsymbol{x}) \right)_\Gamma \approx
  // \sum_{q\in\mathbb{T}_\Gamma} \boldsymbol{v}_i^T (\boldsymbol{x}_q)
  // \sigma \kappa (\boldsymbol{x}_q) \boldsymbol{n} (\boldsymbol{x}_q) \cdot
  // |J(\boldsymbol{x}_q)| w(\boldsymbol{x}_q) \quad \forall
  // i\in\mathbb{T}_\Omega
  // .
  // @f]
  // We decompose this operation in two steps. In the first step, we evaluate
  // the forces $\sigma (\boldsymbol{x}_q) \kappa (\boldsymbol{x}_q)
  // \boldsymbol{n}
  // (\boldsymbol{x}_q)$ at the quadrature points defined on the immersed mesh
  // and weight them with the mapped quadrature weight $|J(\boldsymbol{x}_q)|
  // w_q$:
  // @f[
  // \boldsymbol{F}_S (\boldsymbol{x}_q) \gets \sigma \kappa (\boldsymbol{x}_q)
  // \boldsymbol{n} (\boldsymbol{x}_q) \cdot |J(\boldsymbol{x}_q)| w_q \quad
  // \forall q\in\mathbb{T}_\Gamma.
  // @f]
  // In the second step, we perform the testing on the background mesh:
  // @f[
  // (\boldsymbol v_i (\boldsymbol{x}), \boldsymbol F_S
  ...
  void example_3()
  {
    ...
    // We use a circle with radius $r=0.75$ which is
    ...
    // which is updated in every time step according to the nodal
    // displacements. Two types of finite elements are used to
    // represent scalar and vector-valued DoF values. We renumber
    // the DoFs related to the vector-valued problem to keep the accessing
    // of the individual components simple.
    ...
    // points of the immersed mesh. We throw an assert if one of the points
    ...
    // resulting update of the corrdinates. Finally, we update the mapping of
    ...
    for (unsigned int it = 0; it <= n_time_steps; ++it, time += dt)
      {
        ...
        if (it > 0)
          {
            ...
            dealii::VectorTools::interpolate(mapping_background,
                                             dof_handler_background,
                                             vortex,
                                             velocity);
            ...
            const auto immersed_velocity =
              dealii::VectorTools::point_values<dim>(rpe,
                                                     dof_handler_background,
                                                     velocity);
        ...
        // Next, we loop over all locally owned cells and the quadrature points
        // of the immersed mesh to compute the value for the local surface
        // tension force contribution $\boldsymbol{F}_S(\boldsymbol{x}_q)$. We
        // the real coordinates of the quadrature points and the corresponding
        // force contributions in two individual vectors. For computation of the
        // latter, the normal vector $\boldsymbol{n}(\boldsymbol{x}_q)$ can be
        // directly extracted from the surface mesh via FEValues and for the
        // curvature we use the following approximation
        // @f[
        ...
        // setup from the quadrature points of the immersed mesh, determining
        ...
        // Before utilizing the function of
        // Utilities::MPI::RemotePointEvaluation that performs the
        // multiplication with the test function, we setup a callback function
        ...
        // arbitrary points within a cell. We loop over the cells which surround
        // quadrature points of the immersed mesh -- provided by the callback
        // functions. From the provided CellData object, we retrieve the unit
        // points, i.e., quadrature points of the immersed mesh that lie within
        // the current cell and a pointer to the stored values on the current
        // cell (local surface tension force) for convenience. We reinitialize
        // the data structure of FEPointEvaluation on every cell according to
        // the unit points. Next, we loop over the quadrature points attributed
        // to the cell and submit the local surface tension force to the
        // FEPointEvaluation object. Via FEPointEvaluation::test_and_sum(), the
        // submitted values are multiplied by the values of the test function
        // and a summation over all given points is performed. Subsequently, the
        // contributions are assembled into the global vector containing the
        // weak form of the surface tension force.
