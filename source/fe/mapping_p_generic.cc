  ...
  namespace MappingQ1
  {
    namespace
    {
      ...
      compute_shape_function_values_general(
        ...
        typename dealii::MappingPGeneric<dim, spacedim>::InternalData &data)
      {
      ...
      compute_shape_function_values_hardcode(
        ...
        dealii::MappingPGeneric<1, 1>::InternalData &data)
      {
      ...
      compute_shape_function_values_hardcode(
        ...
        dealii::MappingPGeneric<2, 2>::InternalData &data)
      {
        (void)n_shape_functions;
        const unsigned int n_points = unit_points.size();
        for (unsigned int k = 0; k < n_points; ++k)
      ...
      compute_shape_function_values_hardcode(
        ...
        dealii::MappingPGeneric<3, 3>::InternalData &data)
      {
        (void)n_shape_functions;
        const unsigned int n_points = unit_points.size();
        for (unsigned int k = 0; k < n_points; ++k)
...
template <int dim, int spacedim>
MappingPGeneric<dim, spacedim>::InternalData::InternalData(
...
template <int dim, int spacedim>
std::size_t
MappingPGeneric<dim, spacedim>::InternalData::memory_consumption() const
{
  return (
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::InternalData::initialize(
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::InternalData::initialize_face(
  const UpdateFlags      update_flags,
  const Quadrature<dim> &q,
  const unsigned int     n_original_q_points)
{
...
template <>
void
MappingPGeneric<1, 1>::InternalData::compute_shape_function_values(
  const std::vector<Point<1>> &unit_points)
{
...
template <>
void
MappingPGeneric<2, 2>::InternalData::compute_shape_function_values(
  const std::vector<Point<2>> &unit_points)
{
  // if the polynomial degree is one, then we can simplify code a bit
  // by using hard-coded shape functions.
...
template <>
void
MappingPGeneric<3, 3>::InternalData::compute_shape_function_values(
  const std::vector<Point<3>> &unit_points)
{
  // if the polynomial degree is one, then we can simplify code a bit
  // by using hard-coded shape functions.
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::InternalData::compute_shape_function_values(
  const std::vector<Point<dim>> &unit_points)
{
...
namespace internal
{
  namespace MappingPGenericImplementation
  {
    namespace
    {
      /**
      ...
      compute_mapped_location_of_point(
        const typename dealii::MappingPGeneric<dim, spacedim>::InternalData
          &data)
      {
      ...
      do_transform_real_to_unit_cell_internal(
        ...
        typename dealii::MappingPGeneric<dim, dim>::InternalData &mdata)
      {
            ...
            do
              {
                ...
                Point<spacedim> p_real_trial =
                  internal::MappingPGenericImplementation::
                    compute_mapped_location_of_point<dim, spacedim>(mdata);
      ...
      do_transform_real_to_unit_cell_internal_codim1(
        ...
        const Point<dim + 1> &                                             p,
        const Point<dim> &initial_p_unit,
        typename dealii::MappingPGeneric<dim, dim + 1>::InternalData &mdata)
      {
...
template <int dim, int spacedim>
MappingPGeneric<dim, spacedim>::MappingPGeneric(const unsigned int p)
  : polynomial_degree(p)
...
template <int dim, int spacedim>
MappingPGeneric<dim, spacedim>::MappingPGeneric(
  const MappingPGeneric<dim, spacedim> &mapping)
...
MappingPGeneric<dim, spacedim>::clone() const
{
  return std::make_unique<MappingPGeneric<dim, spacedim>>(*this);
}
...
template <int dim, int spacedim>
unsigned int
MappingPGeneric<dim, spacedim>::get_degree() const
{
...
template <int dim, int spacedim>
Point<spacedim>
MappingPGeneric<dim, spacedim>::transform_unit_to_real_cell(
...
// In the code below, GCC tries to instantiate MappingPGeneric<3,4> when
...
template <int dim, int spacedim>
Point<dim>
MappingPGeneric<dim, spacedim>::transform_real_to_unit_cell_internal(
...
template <>
Point<1>
MappingPGeneric<1, 1>::transform_real_to_unit_cell_internal(
  ...
  const Point<1> &                          initial_p_unit) const
{
  ...
  // spacedim=dim+1, etc
  return internal::MappingPGenericImplementation::
    do_transform_real_to_unit_cell_internal<1>(cell, p, initial_p_unit, *mdata);
}

template <>
Point<2>
MappingPGeneric<2, 2>::transform_real_to_unit_cell_internal(
...
template <>
Point<3>
MappingPGeneric<3, 3>::transform_real_to_unit_cell_internal(
...
template <>
Point<1>
MappingPGeneric<1, 2>::transform_real_to_unit_cell_internal(
...
template <>
Point<2>
MappingPGeneric<2, 3>::transform_real_to_unit_cell_internal(
...
template <>
Point<1>
MappingPGeneric<1, 3>::transform_real_to_unit_cell_internal(
...
template <int dim, int spacedim>
Point<dim>
MappingPGeneric<dim, spacedim>::transform_real_to_unit_cell(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
...
template <int dim, int spacedim>
UpdateFlags
MappingPGeneric<dim, spacedim>::requires_update_flags(
...
MappingPGeneric<dim, spacedim>::get_data(const UpdateFlags      update_flags,
...
template <int dim, int spacedim>
std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>
MappingPGeneric<dim, spacedim>::get_face_data(
  const UpdateFlags          update_flags,
...
template <int dim, int spacedim>
std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>
MappingPGeneric<dim, spacedim>::get_subface_data(
  const UpdateFlags          update_flags,
  const Quadrature<dim - 1> &quadrature) const
{
  std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase> data_ptr =
...
MappingPGeneric<dim, spacedim>::fill_fe_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const CellSimilarity::Similarity                            cell_similarity,
    ...
    &output_data) const
{
  ...
  if (dim > 1 && data.tensor_product_quadrature)
    ...
    {
      internal::MappingPGenericImplementation::maybe_compute_q_points<dim,
                                                                      spacedim>(
        QProjector<dim>::DataSetDescriptor::cell(),
        data,
        output_data.quadrature_points);

      internal::MappingPGenericImplementation::maybe_update_Jacobians<dim,
                                                                      spacedim>(
        computed_cell_similarity,
        QProjector<dim>::DataSetDescriptor::cell(),
        data);

      internal::MappingPGenericImplementation::maybe_update_jacobian_grads<
        dim,
  ...
  namespace MappingPGenericImplementation
  {
    namespace
    {
      ...
      maybe_compute_face_data(
        const dealii::MappingPGeneric<dim, spacedim> &mapping,
      ...
      /**
       * Do the work of MappingPGeneric::fill_fe_face_values() and
       * MappingPGeneric::fill_fe_subface_values() in a generic way,
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::fill_fe_face_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const unsigned int                                          face_no,
  const Quadrature<dim - 1> &                                 quadrature,
  const typename Mapping<dim, spacedim>::InternalDataBase &   internal_data,
    ...
    &output_data) const
{
  ...
  internal::MappingPGenericImplementation::do_fill_fe_face_values(
    *this,
    cell,
    face_no,
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::fill_fe_subface_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const unsigned int                                          face_no,
  const unsigned int                                          subface_no,
  const Quadrature<dim - 1> &                                 quadrature,
    ...
    namespace
    {
        ...
        const ArrayView<Tensor<rank, spacedim>> &                output)
      {
        ...
        Assert((dynamic_cast<const typename dealii::
                               MappingPGeneric<dim, spacedim>::InternalData *>(
                  &mapping_data) != nullptr),
               ExcInternalError());
        const typename dealii::MappingPGeneric<dim, spacedim>::InternalData
          &data =
            static_cast<const typename dealii::MappingPGeneric<dim, spacedim>::
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::transform(
  const ArrayView<const Tensor<1, dim>> &                  input,
  const MappingKind                                        mapping_kind,
  const typename Mapping<dim, spacedim>::InternalDataBase &mapping_data,
  const ArrayView<Tensor<1, spacedim>> &                   output) const
{
  internal::MappingPGenericImplementation::transform_fields(input,
                                                            mapping_kind,
                                                            mapping_data,
                                                            output);
}
...
{
  internal::MappingPGenericImplementation::transform_differential_forms(
  ...
  switch (mapping_kind)
    {
      ...
      case mapping_contravariant_gradient:
      case mapping_covariant_gradient:
        internal::MappingPGenericImplementation::transform_gradients(
          input, mapping_kind, mapping_data, output);
        return;
      default:
        Assert(false, ExcNotImplemented());
  ...
  switch (mapping_kind)
    {
      ...
      case mapping_contravariant_hessian:
      case mapping_covariant_hessian:
        internal::MappingPGenericImplementation::transform_hessians(
          input, mapping_kind, mapping_data, output);
        return;
      default:
        Assert(false, ExcNotImplemented());
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::add_line_support_points(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
...
template <>
void
MappingPGeneric<3, 3>::add_quad_support_points(
  const Triangulation<3, 3>::cell_iterator &cell,
...
template <>
void
MappingPGeneric<2, 3>::add_quad_support_points(
  const Triangulation<2, 3>::cell_iterator &cell,
  std::vector<Point<3>> &                   a) const
{
...
template <int dim, int spacedim>
void
MappingPGeneric<dim, spacedim>::add_quad_support_points(
  const typename Triangulation<dim, spacedim>::cell_iterator &,
...
MappingPGeneric<dim, spacedim>::compute_mapping_support_points(
