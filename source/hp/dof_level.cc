...
namespace internal
{
  namespace hp
  {
    ...
    //                   ExcInternalError());
    //
    //            // see if the range of dofs for this cell can be compressed
    //            and if
    //            // so how many slots we have to store for them
    //            if (next_offset > dof_offsets[cell])
    //              {
    //                bool compressible = true;
    //                for (unsigned int j = dof_offsets[cell] + 1; j <
    //                next_offset;
    //                     ++j)
    ...
    //                    }
    //
    //                // if this cell is compressible, then copy the first index
    //                and
    ...
    //
    //                    // make sure that the current active_fe_index
    //                    indicates
    ...
    //                  }
    //                else
    //                  for (unsigned int i = dof_offsets[cell]; i <
    //                  next_offset; ++i)
    ...
    //        if (dof_offsets[cell] != static_cast<offset_type>(-1))
    //          {
    //            // we know now that the slot for this cell is used. extract
    //            the
    ...
    //                           .template n_dofs_per_object<dim>(),
    //                       ExcInternalError());
    //                for (unsigned int i = dof_offsets[cell]; i < next_offset;
    //                ++i)
    //                  new_dof_indices.push_back(dof_indices[i]);
    //              }
    //            else
