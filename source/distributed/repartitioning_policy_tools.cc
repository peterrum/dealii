...
namespace RepartitioningPolicyTools
{
    ...
    const Triangulation<dim, spacedim> &tria_in) const
  {
    ...
    // ... partial sum of each weight
    if (partition.locally_owned_size() > 0)
      partition.local_element(0) = process_local_weight_offset;

    for (unsigned int i = 1; i < partition.locally_owned_size(); ++i)
      partition.local_element(i) =
        partition.local_element(i - 1) + static_cast<double>(weights[i - 1]);

    // ... convert partial sum to rank of cell
    for (unsigned int i = 0; i < partition.locally_owned_size(); ++i)
      partition.local_element(i) =
        partition.local_element(i) * n_subdomains / total_weight;
