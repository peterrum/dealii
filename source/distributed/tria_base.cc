...
#  endif
  ...
  TriangulationBase<dim, spacedim>::reset_global_cell_indices()
  {
    ...
    GridTools::exchange_cell_data_to_ghosts<types::global_cell_index>(
      *this,
      [](const auto &cell) { return cell->global_active_cell_index_local(); },
    ...
    for (const auto &cell : this->active_cell_iterators())
      if (!cell->is_artificial())
        {
          const auto index = cell->global_active_cell_index_local();
    ...
    for (const auto &cell : this->active_cell_iterators())
      if (!cell->is_artificial())
        {
          if (cell->global_active_cell_index_local() !=
              numbers::invalid_dof_index)
            cell->set_global_active_cell_index(
              number_cache.active_cell_index_partitioner.global_to_local(
                cell->global_active_cell_index_local()));
        }
    ...
    if (this->is_multilevel_hierarchy_constructed() == true)
      {
        ...
        GridTools::exchange_cell_data_to_level_ghosts<
          ...
          *this,
          [](const auto &cell) {
            return cell->global_level_cell_index_local();
          },
          [](const auto &cell, const auto &id) {
        ...
        for (unsigned int l = 0; l < this->n_global_levels(); ++l)
          {
            ...
            for (const auto &cell : this->cell_iterators_on_level(l))
              if (cell->level_subdomain_id() !=
                ...
                {
                  const auto index = cell->global_level_cell_index_local();
            ...
            for (const auto &cell : this->cell_iterators_on_level(l))
              if (cell->level_subdomain_id() !=
                  dealii::numbers::artificial_subdomain_id)
                {
                  if (cell->global_level_cell_index_local() !=
                      numbers::invalid_dof_index)
                    cell->set_global_level_cell_index(
                      number_cache.level_cell_index_partitioners[l]
                        .global_to_local(
                          cell->global_level_cell_index_local()));
                }
          }
      }
