...
namespace ReferenceCell
{
  ...
  void
  create_coarse_triangulation(Triangulation<dim, spacedim> &tria,
                              const Type &                  reference_cell)
  {
    if (reference_cell == get_hypercube(dim))
      ...
      {
        std::vector<Point<spacedim>> vertices(3);
    ...
    else if (reference_cell == Type::Tet)
      {
        std::vector<Point<spacedim>> vertices = {
          {Point<spacedim>(0.0, 0.0, 0.0),
           Point<spacedim>(1.0, 0.0, 0.0),
           Point<spacedim>(0.0, 1.0, 0.0),
           Point<spacedim>(0.0, 0.0, 1.0)}};
    ...
    else if (reference_cell == Type::Wedge)
      {
        std::vector<Point<spacedim>> vertices = {
          {Point<spacedim>(1.0, 0.0, 0.0),
           Point<spacedim>(0.0, 1.0, 0.0),
           Point<spacedim>(0.0, 0.0, 0.0),
           Point<spacedim>(1.0, 0.0, 1.0),
           Point<spacedim>(0.0, 1.0, 1.0),
           Point<spacedim>(0.0, 0.0, 1.0)}};
  ...
  template <int dim>
  Quadrature<dim>
  get_default_quadrature(const Type &reference_cell, const unsigned n_points_1D)
  {
    if (reference_cell == get_hypercube(dim))
