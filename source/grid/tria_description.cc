...
namespace TriangulationDescription
{
  namespace Utilities
  {
    ...
    namespace
    {
      ...
      namespace internal
      {
                  ...
                  for (const auto v : cell->vertex_indices())
                    {
                      ...
                      if (coinciding_vertex_group !=
                        ...
                        for (const auto &co_vertex :
                             coinciding_vertex_groups.at(
                               coinciding_vertex_group->second))
                          vertices_owned_by_locally_owned_cells[co_vertex] =
                            true;
                    }
                };
        ...
        private:
          void
          set_additional_data(
            Description<dim, spacedim> &construction_data) const
          {
      ...
      const unsigned int                          my_rank_in)
    {
      ...
      return internal::CreateDescriptionFromTriangulationHelper<dim, spacedim>(
               tria,
               subdomain_id_function,
               level_subdomain_id_function,
               comm,
      ...
      const TriangulationDescription::Settings settings_in)
    {
      ...
      internal::CreateDescriptionFromTriangulationHelper<dim, spacedim> helper(
        tria,
        subdomain_id_function,
        level_subdomain_id_function,
        tria.get_communicator(),
      ...
      description_temp.reserve(relevant_processes.size());

      for (const auto rank : relevant_processes)
        description_temp.emplace_back(
