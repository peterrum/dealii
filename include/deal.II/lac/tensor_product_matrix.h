...
public:
   ...
   * increased memory consumption if many instances of this class is created,
   * e.g., in the case of non-Cartesian meshes. Futhermore, only one thread run
   * this function at once (ensured internally with a mutex). If these two
   * limitations are an issue for you, please consider the other version of this
   * function.
   */
  void
  ...
  const ArrayView<const Number> &src_view) const
{
  ...
  vmult(dst_view, src_view, this->tmp_array);
}
