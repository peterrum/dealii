 ...
 * always use MappingPGeneric over the existing class MappingQ (which, unless
 ...
 * the behavior of MappingQ. Of course, MappingPGeneric also uses bilinear
...
class MappingPGeneric : public Mapping<dim, spacedim>
{
public:
  /**
   ...
   * cell.
   */
  MappingPGeneric(const unsigned int polynomial_degree);

  /**
   * Copy constructor.
   */
  MappingPGeneric(const MappingPGeneric<dim, spacedim> &mapping);
...
protected:
  ...
  // functions on its MappingPGeneric(1) sub-object.
  template <int, int>
...
inline const double &
MappingPGeneric<dim, spacedim>::InternalData::shape(
...
template <int dim, int spacedim>
inline double &
MappingPGeneric<dim, spacedim>::InternalData::shape(const unsigned int qpoint,
...
MappingPGeneric<dim, spacedim>::InternalData::derivative(
  const unsigned int qpoint,
  const unsigned int shape_nr) const
{
...
MappingPGeneric<dim, spacedim>::InternalData::second_derivative(
  const unsigned int qpoint,
  const unsigned int shape_nr) const
{
  AssertIndexRange(qpoint * n_shape_functions + shape_nr,
...
MappingPGeneric<dim, spacedim>::InternalData::third_derivative(
  const unsigned int qpoint,
  const unsigned int shape_nr) const
{
  AssertIndexRange(qpoint * n_shape_functions + shape_nr,
...
MappingPGeneric<dim, spacedim>::InternalData::fourth_derivative(
  const unsigned int qpoint,
  const unsigned int shape_nr) const
{
  AssertIndexRange(qpoint * n_shape_functions + shape_nr,
...
template <int dim, int spacedim>
inline bool
MappingPGeneric<dim, spacedim>::preserves_vertex_locations() const
{
  return true;
}
