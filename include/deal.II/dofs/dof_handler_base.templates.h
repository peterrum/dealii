...
namespace internal
{
  namespace hp
  {
    namespace DoFHandlerImplementation
    {
        ...
        DoFHandlerBase<dim, spacedim, T> &dof_handler)
      {
        ...
        for (unsigned int level = 0; level < dof_handler.tria->n_levels();
             ++level)
          {
            ...
            Assert(dof_handler.levels_hp[level]->dof_indices.size() == counter,
                   ExcInternalError());
                    ...
                    // index:
                      ...
                      n_face_slots +=
                        (1 + // the active_fe_index
                         dof_handler.get_fe(cell->active_fe_index())
                           .template n_dofs_per_object<dim - 1>() // actual DoF
                                                                  // indices
                    ...
                    if (cell->at_boundary(face) ||
                    ...
                    else
                      {
                        ...
                        if (active_fe_indices[1] < active_fe_indices[0])
                          std::swap(active_fe_indices[0], active_fe_indices[1]);
      ...
      template <int spacedim>
      unsigned int
      Implementation::max_couplings_between_dofs(
        const DoFHandlerBase<1, spacedim, dealii::hp::DoFHandler<1, spacedim>>
          &dof_handler)
      {
        return std::min(static_cast<types::global_dof_index>(
      ...
      unsigned int
      Implementation::max_couplings_between_dofs(
        const DoFHandlerBase<2, spacedim, dealii::hp::DoFHandler<2, spacedim>>
          &dof_handler)
      {
      ...
      unsigned int
      Implementation::max_couplings_between_dofs(
        const DoFHandlerBase<3, spacedim, dealii::hp::DoFHandler<3, spacedim>>
          &dof_handler)
      {
      ...
      template <int dim, int spacedim>
      unsigned int
      Implementation::determine_fe_from_children(
        const std::vector<unsigned int> &        children_fe_indices,
        dealii::hp::FECollection<dim, spacedim> &fe_collection)
      {
...
namespace internal
{
  ...
  namespace DoFHandlerImplementation
  {
    ...
    template <int spacedim, typename T>
    void Implementation::reserve_space_mg(
      DoFHandlerBase<1, spacedim, T> &dof_handler)
    {
    ...
    template <int spacedim, typename T>
    void Implementation::reserve_space_mg(
      DoFHandlerBase<2, spacedim, T> &dof_handler)
    {
      Assert(dof_handler.get_triangulation().n_levels() > 0,
             ExcMessage("Invalid triangulation"));
      dof_handler.clear_mg_space();
    ...
    template <int spacedim, typename T>
    void Implementation::reserve_space_mg(
      DoFHandlerBase<3, spacedim, T> &dof_handler)
    {
      Assert(dof_handler.get_triangulation().n_levels() > 0,
             ExcMessage("Invalid triangulation"));
      dof_handler.clear_mg_space();
    ...
    Implementation::get_dof_index(
      const DoFHandlerBase<dim, spacedim, dealii::hp::DoFHandler<dim, spacedim>>
        &,
      const std::unique_ptr<internal::DoFHandlerImplementation::DoFLevel<dim>>
        &,
      const std::unique_ptr<internal::DoFHandlerImplementation::DoFFaces<dim>>
