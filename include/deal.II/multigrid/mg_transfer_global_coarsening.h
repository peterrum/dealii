...
public:
   ...
   */
  void
  prolongate(const unsigned int, VectorType &dst, const VectorType &src) const;
...
private:
  ...
  struct MGTransferScheme
  {
     ...
     * the multiplicity of DoF shares and constraints have to be taken in
     * account via weights.
     */
...
public:
   ...
   * transfer operators (with the coarsest level kept empty).
   */
