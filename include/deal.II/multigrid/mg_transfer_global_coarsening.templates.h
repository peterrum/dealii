...
{
  const auto is_contained = [](const auto &partioner,
                               const auto &external_partitioner) -> bool {
  ...
  else if (is_contained(this->partitioner_coarse, external_partitioner_coarse))
    {
      this->vec_coarse.reinit(0);
  ...
  else if (is_contained(this->partitioner_fine, external_partitioner_fine))
    {
      this->vec_fine.reinit(0);
