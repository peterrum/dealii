...
namespace internal
{
  ...
  struct CellwiseInverseMassMatrixImplTransformFromQPoints
  {
        ...
        typename std::enable_if<fe_degree == -1>::type * = nullptr)
    {
      ...
      AlignedVector<Number> temp_1(std::max(n_q_points, dofs_per_component));
      AlignedVector<Number> temp_2(std::max(n_q_points, dofs_per_component));
      ...
      for (unsigned int d = 0; d < n_desired_components; ++d)
        {
          ...
          if (dim == 3)
            {
              evaluator.template hessians<2, false, false>(in, temp_1.begin());
              evaluator.template hessians<1, false, false>(temp_1.begin(),
                                                           temp_2.begin());
              evaluator.template hessians<0, false, false>(temp_2.begin(), out);
            }
          if (dim == 2)
            {
              evaluator.template hessians<1, false, false>(in, temp_1.begin());
              evaluator.template hessians<0, false, false>(temp_1.begin(), out);
            }
          if (dim == 1)
            evaluator.template hessians<0, false, false>(in, out);
        }
