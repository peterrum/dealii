    ...
    if (is_non_buffering_sm_supported)
      {
        ...
        const auto procs_of_sm = [](const MPI_Comm &comm,
          ...
          MPI_Allgather(
            &rank, 1, MPI_UNSIGNED, ranks.data(), 1, MPI_INT, comm_shared);
            ...
            AnonymousProcess<dealii::types::global_dof_index, unsigned int>
              process(
                ...
                [&](const auto &other_rank,
                  ...
                  for (unsigned int i = 0; i < buffer_recv.size(); ++i, ++j)
                    {
                      for (; j < cells_locally_owned.size(); ++j)
                        if (cells_locally_owned[j].first == buffer_recv[i])
                          {
        ...
        for (unsigned int i = 0; i < 2; ++i)
          {
            cell_indices_contiguous_sm[i].resize(
            ...
            for (unsigned int face_index = 0;
                 ...
                 ++face_index)
              {
                const unsigned int cell_index =
                  ...
                  : get_face_info(face_index / n_lanes).cells_exterior[face_index % n_lanes];
        ...
        for (auto &di : dof_info)
          di.compress_for_shared_memory_usage(cell_indices_contiguous_sm);
      }
  }
#endif
