...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    public:
      ...
      std::vector<internal::MatrixFreeFunctions::ShapeInfo<Number>> shape_infos;
      ...
      const std::shared_ptr<const Utilities::MPI::Partitioner> &    partitioner)
    {
      ...
      if (hanging_nodes)
        {
          ...
          std::vector<internal::MatrixFreeFunctions::ConstraintKinds> mask(
      ...
      for (unsigned int i = 0; i < local_dof_indices_lex.size(); ++i)
        {
          types::global_dof_index current_dof = local_dof_indices_lex[i];
      ...
      AlignedVector<Number> &evaluation_data_coarse) const
    {
      ...
      std::array<internal::MatrixFreeFunctions::ConstraintKinds, Number::size()>
        constraint_mask;
