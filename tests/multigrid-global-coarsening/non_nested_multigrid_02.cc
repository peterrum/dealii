...
//   MGLevelObject<Triangulation<dim>>        triangulations(min_level,
//   max_level); MGLevelObject<DoFHandler<dim>> dof_handlers(min_level,
//   max_level); MGLevelObject<AffineConstraints<Number>> constraints(min_level,
//   max_level); MGLevelObject<std::unique_ptr<Mapping<dim>>>
//   mappings(min_level, max_level);
//   MGLevelObject<std::shared_ptr<MGTwoLevelTransferNonNested<dim,
//   VectorType>>>
...
//       test<2>(n_refinements, degree, factor, false /*quad*/);

//   for (unsigned int n_refinements = 2; n_refinements <= 4; ++n_refinements)
//     for (unsigned int degree = 1; degree <= 2; ++degree)
//       test<2>(n_refinements, degree, factor, true /*triangle*/);
// }
// ---------------------------------------------------------------------
//
// Copyright (C) 2020 - 2022 by the deal.II authors
