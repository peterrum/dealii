...
do_test(const DoFHandler<dim> &dof, const unsigned int nb)
{
  ...
  MGTransferBlockGlobalCoarsening<dim, LinearAlgebra::distributed::Vector<number>> mg_transfer(mg_constrained_dofs);
  ...
  PreconditionMG<dim,
                 LinearAlgebra::distributed::BlockVector<number>,
                 MGTransferBlockGlobalCoarsening<dim, LinearAlgebra::distributed::Vector<number>>>
