...
// Test FETools::compute_embedding_matrices for simplices.
...
main()
{
  ...
  matrices[RefinementCase<dim>::isotropic_refinement - 1] =
    std::vector<FullMatrix<double>>(4,
                                    FullMatrix<double>(fe.n_dofs_per_cell(),
                                                       fe.n_dofs_per_cell()));
  FETools::compute_embedding_matrices(fe, matrices, true);
  ...
  for (const auto &cell_coarse : dof_handler_coarse.active_cell_iterators())
    {
      ...
      for (unsigned int c = 0; c < cell_coarse_on_fine_tria.n_children(); ++c)
        {
          matrices[RefinementCase<dim>::isotropic_refinement - 1][c].vmult(
            temp_fine, temp_coarse);
