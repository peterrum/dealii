...
test(const PreconditionerType &preconditioner, const VectorType &src)
{
  ...
  return {norm_0, norm_1, norm_2, norm_3};
}
...
main()
{
  ...
  std::vector<unsigned int> n_smoothing_stepss{1, 2, 3};

  for (const auto relaxation : relaxations)
    for (const auto n_smoothing_steps : n_smoothing_stepss)
      {
...

        {
          ...
          ad.n_smoothing_steps = n_smoothing_steps;
