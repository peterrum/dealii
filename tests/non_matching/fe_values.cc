...
{
  AssertThrow(expected->level() == cell->level(), ExcInternalError());
  AssertThrow(expected->index() == cell->index(), ExcInternalError());
  deallog << "OK" << std::endl;
}
...
void
assert_doesnt_have_value(const boost::optional<const FEVALUES &> &fe_values)
{
